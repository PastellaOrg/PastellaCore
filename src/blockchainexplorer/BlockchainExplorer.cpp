...
namespace Pastella
{
    ...
    bool BlockchainExplorer::addObserver(IBlockchainObserver *observer)
    {
        ...
        {
            throw std::system_error(make_error_code(Pastella::error::BlockchainExplorerErrorCodes::NOT_INITIALIZED));
        }
    ...
    void BlockchainExplorer::init()
    {
        if (state.load() != NOT_INITIALIZED)
        {
            ...
            throw std::system_error(
                make_error_code(Pastella::error::BlockchainExplorerErrorCodes::ALREADY_INITIALIZED));
        }

        if (!getBlockchainTop(knownBlockchainTop, false))
        {
            logger(ERROR) << "Can't get blockchain top.";
            throw std::system_error(make_error_code(Pastella::error::BlockchainExplorerErrorCodes::INTERNAL_ERROR));
        }
        ...
        uint32_t &blocksNumberWithinTimestamps)
    {
        ...
        {
            throw std::system_error(
                make_error_code(Pastella::error::BlockchainExplorerErrorCodes::REQUEST_ERROR),
