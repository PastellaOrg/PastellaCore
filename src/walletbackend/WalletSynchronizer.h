...
struct BlockScanTmpInfo
{
    ...
    std::vector<std::tuple<Crypto::PublicKey, Crypto::Hash, uint64_t>> keyImagesToMarkSpent;
};
...
class WalletSynchronizer
{
  ...
  private:
    ...
    std::vector<std::tuple<Crypto::PublicKey, WalletTypes::TransactionInput>>
        processTransactionOutputs(const WalletTypes::RawTransaction &rawTX, const uint64_t blockHeight) const;
