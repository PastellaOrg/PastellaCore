...
{
    /* VIEW KEY REMOVAL: Generate address from private key only */
    std::string address = Utilities::privateKeyToAddress(privateKey);

    m_eventHandler = std::make_shared<EventHandler>();

    m_subWallets = std::make_shared<SubWallets>(privateKey, address, scanHeight, newWallet);
}
    ...
    const unsigned int syncThreadCount)
{
    ...
    /* Convert the mnemonic into a private key */
    auto [mnemonicError, privateKey] = Mnemonics::MnemonicToPrivateKey(mnemonicSeed);
    ...
    const unsigned int syncThreadCount)
{
    ...
    if (Error error = validatePrivateKey(privateKey); error != SUCCESS)
    {
        return {error, nullptr};
    }
...
/* VIEW WALLET IMPORT REMOVED: importViewWallet() no longer needed in transparent system */
    ...
    const unsigned int syncThreadCount)
{
    ...
    Pastella::KeyPair publicKey;

    /* VIEW KEY REMOVAL: Generate only private key */
    ...
    const uint16_t daemonPort)
{
    try
    {
        ...
        const Pastella::Currency currency = Pastella::CurrencyBuilder(logManager).currency();

        System::Dispatcher localDispatcher;
        System::Dispatcher *dispatcher = &localDispatcher;

        /* Our connection to Pastellad */
        std::unique_ptr<Pastella::INode> node(new Pastella::NodeRpcProxy(daemonHost, daemonPort, 10, logManager));
        ...
        Pastella::WalletGreen wallet(*dispatcher, currency, *node, logManager);
...
std::tuple<Error, uint64_t, uint64_t> WalletBackend::getBalance(const std::string address) const
{
    ...
    const auto [unlockedBalance, lockedBalance] = m_subWallets->getBalance(
        Utilities::addressesToPublicKeys({address}), takeFromAll, m_daemon->networkBlockCount());
    ...
    const bool sendTransaction)
{
    ...
    return std::make_tuple(error, hash, preparedTransaction);
}
...
std::tuple<Error, Crypto::Hash, WalletTypes::PreparedTransactionInfo, std::string> WalletBackend::stake(
    const uint64_t amount,
    ...
    const bool sendTransaction)
{
    ...
    const auto [error, hash, preparedTransaction, rewardAddress] = SendTransaction::sendStakingTransaction(
        amount,
        lockDurationDays,
        address,
        m_daemon,
    ...
    return std::make_tuple(error, hash, preparedTransaction, rewardAddress);
}



std::vector<WalletTypes::StakeInfo> WalletBackend::getUserStakes() const
{
    /* Get all wallet transactions to find staking transactions */
    const auto transactions = getTransactions();

    /* Extract staking transaction hashes */
    std::vector<std::string> stakingHashes;
    for (const auto& transaction : transactions)
    {
        if (transaction.isStakingTransaction)
        {
            stakingHashes.push_back(Common::podToHex(transaction.hash));
        }
    }

    std::cout << "WALLET DEBUG - Found " << stakingHashes.size() << " staking transactions in wallet history" << std::endl;

    /* If no staking transactions found, return empty list */
    if (stakingHashes.empty())
    {
        return {};
    }

    /* Use getUserStakesByHashes with the extracted staking transaction hashes */
    return getUserStakesByHashes(stakingHashes);
}

std::vector<WalletTypes::StakeInfo> WalletBackend::getUserStakesByHashes(const std::vector<std::string> &stakingHashes) const
{
    ...
    /* Query the daemon for stakes by hashes */
    const auto [success, daemonStakes] = m_daemon->getUserStakesByHashes(stakingHashes);

    if (!success)
    {
        /* Failed to get stakes from daemon, return empty list */
        return stakes;
    }

    /* Convert daemon stakes to wallet format with current height */
    const uint64_t currentHeight = m_daemon->networkBlockCount();

    for (const auto& daemonStake : daemonStakes)
    {
        WalletTypes::StakeInfo stakeInfo;
        stakeInfo.stakingTxHash = daemonStake.stakingTxHash;
        stakeInfo.amount = daemonStake.amount;
        stakeInfo.unlockTime = daemonStake.unlockTime;
        stakeInfo.lockDurationDays = daemonStake.lockDurationDays;
        stakeInfo.address = daemonStake.address;
        stakeInfo.currentHeight = currentHeight;

        /* Determine if stake is active based on current height vs unlock time */
        stakeInfo.isActive = daemonStake.isActive && (currentHeight < daemonStake.unlockTime);

        /* Use pending rewards from daemon */
        stakeInfo.pendingRewards = daemonStake.pendingRewards;

        /* Copy enhanced reward calculation fields from daemon */
        stakeInfo.blocksStaked = daemonStake.blocksStaked;
        stakeInfo.dailyRewardRate = daemonStake.dailyRewardRate;
        stakeInfo.estDailyReward = daemonStake.estDailyReward;
        stakeInfo.estWeeklyReward = daemonStake.estWeeklyReward;
        stakeInfo.estMonthlyReward = daemonStake.estMonthlyReward;
        stakeInfo.estYearlyReward = daemonStake.estYearlyReward;
        stakeInfo.accumulatedEarnings = daemonStake.accumulatedEarnings;

        /* Extract reward address from transaction extra data */
        stakeInfo.rewardAddress = "";

        /* Get the transaction to extract reward address from extra data */
        const auto [txSuccess, rawTransactionBytes] = m_daemon->getTransaction(daemonStake.stakingTxHash);
        if (txSuccess && !rawTransactionBytes.empty())
        {
            try
            {
                /* Parse raw transaction bytes into transaction structure */
                Pastella::Transaction transaction = Pastella::fromBinaryArray<Pastella::Transaction>(rawTransactionBytes);

                /* Extract staking data from transaction extra */
                /* TRANSPARENT SYSTEM: rewardAddress removed - staker's address tracked in StakingEntry */
                Pastella::TransactionExtraStaking stakingExtra;
                if (Pastella::getStakingDataFromExtra(transaction.extra, stakingExtra))
                {
                    // rewardAddress no longer exists in staking extra
                    // stakeInfo.rewardAddress will remain empty (not used in transparent system)
                }
            }
            catch (const std::exception& e)
            {
                /* Failed to parse transaction or extract extra data - continue with empty reward address */
            }
        }
...
uint64_t WalletBackend::getPendingRewards() const
{
    const auto stakes = getUserStakes();
...
uint64_t WalletBackend::getTotalStaked() const
{
    const auto stakes = getUserStakes();
...
bool WalletBackend::hasActiveStake() const
{
    const auto stakes = getUserStakes();
...
std::tuple<Error, std::string> WalletBackend::createProposal(
    ...
    const std::string recipientAddress)
{
    ...
    /* Get next proposal ID from daemon */
    const auto [success, proposalId] = m_daemon->getNextProposalId();

    if (!success)
    {
        return {DAEMON_ERROR, "0"};
    }

    /* Create governance proposal transaction using the new transaction-based approach */
    const auto [error, txHash, preparedTx] = SendTransaction::sendGovernanceProposalTransaction(
        title,
        description,
        proposalType,
        amount,
    ...
    /* Return transaction hash as proposal ID */
    return {SUCCESS, Common::podToHex(txHash)};
}

std::tuple<Error, std::string> WalletBackend::castVote(
    const uint64_t proposalId,
    const uint8_t vote)
{
    std::scoped_lock lock(m_transactionMutex);

    /* Get voting power (stake weight) */
    const auto [error, votingPower, stakes] = getVotingPower();

    if (error)
    {
        return {error, "Failed to get voting power"};
    }

    if (votingPower == 0)
    {
        return {STAKING_AMOUNT_TOO_LOW, "No active stakes found"};
    }

    /* Create governance vote transaction using the new transaction-based approach */
    const auto [txError, txHash, preparedTx] = SendTransaction::sendGovernanceVoteTransaction(
        proposalId,
        vote,
        votingPower,
        m_daemon,
    ...
    if (txError)
    {
        return {txError, "Failed to cast vote"};
    }
...
std::tuple<Error, std::vector<WalletTypes::GovernanceProposal>> WalletBackend::getGovernanceProposals(
    const bool activeOnly) const
{
    const auto [success, proposals] = m_daemon->getGovernanceProposals(activeOnly);
...
std::tuple<Error, WalletTypes::GovernanceProposal> WalletBackend::getGovernanceProposal(
    const uint64_t proposalId) const
{
    const auto [success, proposal, votes] = m_daemon->getGovernanceProposal(proposalId);

    if (!success)
    {
        return {DAEMON_ERROR, WalletTypes::GovernanceProposal()};
    }

    return {SUCCESS, proposal};
}

std::tuple<Error, std::vector<WalletTypes::GovernanceVote>> WalletBackend::getGovernanceProposalVotes(
    const uint64_t proposalId) const
{
    const auto [success, proposal, votes] = m_daemon->getGovernanceProposal(proposalId);
...
std::tuple<Error, uint64_t, std::vector<WalletTypes::StakeInfo>> WalletBackend::getVotingPower() const
{
    const std::string address = getPrimaryAddress();

    const auto [success, votingPower, stakes] = m_daemon->getVotingPower(address);
    ...
    return {SUCCESS, votingPower, stakes};
}
...
std::tuple<Error, std::string>
    WalletBackend::importSubWallet(const Crypto::SecretKey privateKey, const uint64_t scanHeight)
{
    if (Error error = validatePrivateKey(privateKey); error != SUCCESS)
    {
    ...
    return m_syncRAIIWrapper->pauseSynchronizerToRunFunction([&, this]() {
        /* Add the sub wallet */
        const auto [error, address] = m_subWallets->importSubWallet(privateKey, scanHeight);
...
std::tuple<Error, std::string>
    WalletBackend::importViewSubWallet(const Crypto::PublicKey publicKey, const uint64_t scanHeight)
{
    if (Error error = validatePublicKey(publicKey); error != SUCCESS)
    {
        return {error, std::string()};
    }
    ...
    return m_syncRAIIWrapper->pauseSynchronizerToRunFunction([&, this]() {
        /* Add the sub wallet */
        const auto [error, address] = m_subWallets->importViewSubWallet(publicKey, scanHeight);
...
std::tuple<Error, Crypto::PublicKey, Crypto::SecretKey, uint64_t> WalletBackend::getKeys(const std::string &address) const
{
    const bool allowIntegratedAddresses = false;
    ...
    const auto [success, privateKey, walletIndex] = m_subWallets->getPrivateKey(publicKey);

    return {success, publicKey, privateKey, walletIndex};
}

/* Returns the private key for the primary address */
Crypto::SecretKey WalletBackend::getPrimaryAddressPrivateKey() const
{
    return m_subWallets->getPrimaryKey();
}
...
std::tuple<Error, std::string> WalletBackend::getMnemonicSeedForAddress(const std::string &address) const
{
    ...
    const auto [error, publicKey, privateKey, walletIndex] = getKeys(address);
    ...
    return {SUCCESS, Mnemonics::PrivateKeyToMnemonic(privateKey)};
}
...
std::shared_ptr<Nigel> WalletBackend::getDaemon() const
{
    return m_daemon;
}

std::tuple<Error, std::string> WalletBackend::getAddress(const Crypto::PublicKey publicKey) const
{
    if (Error error = validatePublicKey(publicKey); error != SUCCESS)
    {
        return {error, std::string()};
    ...
    return m_subWallets->getAddress(publicKey);
}
