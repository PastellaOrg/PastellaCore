...
namespace Pastella
{
    void serialize(TransactionInformation &ti, Pastella::ISerializer &s)
    {
        s(ti.transactionHash, "");
        /* STEALTH ADDRESS REMOVAL: keyImage field renamed to publicKey */
        s(ti.publicKey, "");
    ...
    /* STEALTH ADDRESS REMOVAL: Field name keyImage stays same but type is PublicKey */
        ...
        m_globalOutputIndex(0)
    {
        ...
        {
            m_publicKey = &transactionInfo.keyImage;
        }
        else
        {
            assert(false);
    ...
    SpentOutputDescriptor::SpentOutputDescriptor(const Crypto::PublicKey *publicKey)
    {
        assign(publicKey);
    }
    ...
    /* STEALTH ADDRESS REMOVAL: Changed KeyImage to PublicKey */
    void SpentOutputDescriptor::assign(const Crypto::PublicKey *publicKey)
    {
        m_type = TransactionTypes::OutputType::Key;
    ...
    bool SpentOutputDescriptor::operator==(const SpentOutputDescriptor &other) const
    {
        ...
        if (m_type == TransactionTypes::OutputType::Key)
        {
            if (m_publicKey != nullptr && other.m_publicKey != nullptr)
            {
                /* Both have publicKey pointers - compare keys */
                return *m_publicKey == *other.m_publicKey;
            }
            else if (m_publicKey == nullptr && other.m_publicKey == nullptr)
            {
                /* Both use amount+index - compare those */
                return m_amount == other.m_amount && m_globalOutputIndex == other.m_globalOutputIndex;
            }
        }
    ...
    size_t SpentOutputDescriptor::hash() const
    {
        if (m_type == TransactionTypes::OutputType::Key)
        {
            ...
            {
                /* Hash by public key */
                static_assert(sizeof(size_t) < sizeof(*m_publicKey), "sizeof(size_t) < sizeof(*m_publicKey)");
                return *reinterpret_cast<const size_t *>(m_publicKey->data);
            }
            else
            {
                /* Hash by amount+index */
                size_t h1 = std::hash<uint64_t>{}(m_amount);
                size_t h2 = std::hash<uint32_t>{}(m_globalOutputIndex);
                return h1 ^ (h2 << 1);
            }
        }
        else
        ...
        const std::vector<TransactionOutputInformationIn> &transfers)
    {
        ...
        catch (...)
        {
            if (m_transactions.count(tx.getTransactionHash()) == 0)
            {
                m_logger(ERROR) << "Failed to add transaction, remove transaction transfers, block "
    ...
    bool TransfersContainer::addTransactionInputs(const TransactionBlockInfo &block, const ITransactionReader &tx)
    {
        ...
        for (size_t i = 0; i < tx.getInputCount(); ++i)
        {
            ...
            if (inputType == TransactionTypes::InputType::Key)
            {
                ...
                /* TRANSPARENT SYSTEM: Find the output being spent by UTXO reference
                 * KeyInput now explicitly contains transactionHash and outputIndex
                 * We can directly look up which wallet output is being spent */
                SpentOutputDescriptor descriptor(input.amount, input.outputIndex);

                /* Check if this input spends one of our wallet's available outputs */
                ...
                if (availableCount > 0)
                {
                    /* Found matching output(s) - verify exact match by transaction hash */
                    bool found = false;
                    TransactionOutputInformationEx spentOutput;

                    for (auto it = availableRange.first; it != availableRange.second; ++it)
                    {
                        if (it->transactionHash == input.transactionHash &&
                            it->outputInTransaction == input.outputIndex)
                        {
                            spentOutput = *it;
                            found = true;
                            break;
                    ...
                    if (found)
                    {
                        /* Check for double-spend within this container */
                        auto spentRange = m_spentTransfers.get<SpentOutputDescriptorIndex>().equal_range(descriptor);
                        if (std::distance(spentRange.first, spentRange.second) > 0)
                        {
                            m_logger(ERROR)
                                << "Failed to add key input: output already spent, "
                                << "tx hash " << input.transactionHash << ", output index " << input.outputIndex;
                            throw std::runtime_error("Output already spent");
                        }

                        /* Move output from available to spent */
                        copyToSpent(block, tx, i, spentOutput);

                        /* Remove from available transfers */
                        auto &outputDescriptorIndex = m_availableTransfers.get<SpentOutputDescriptorIndex>();
                        outputDescriptorIndex.erase(spentOutput);
        ...
        const std::vector<uint32_t> &globalIndices)
    {
        ...
        try
        {
            ...
            for (auto transferIt = availableRange.first; transferIt != availableRange.second;)
            {
                ...
                if (transfer.outputInTransaction >= globalIndices.size())
                {
                    ...
                    m_logger(ERROR) << message << ", globalIndices.size() " << globalIndices.size()
        ...
        {
            m_logger(ERROR) << "markTransactionConfirmed failed: " << e.what()
    ...
    void TransfersContainer::deleteTransactionTransfers(const Hash &transactionHash)
    {
        ...
        for (auto it = unconfirmedTransfersRange.first; it != unconfirmedTransfersRange.second;)
        {
            ...
            {
                /* STEALTH ADDRESS REMOVAL: Changed KeyImage to PublicKey */
                Crypto::PublicKey keyImage = it->keyImage;
     ...
     */
    /* STEALTH ADDRESS REMOVAL: Changed KeyImage to PublicKey */
    void TransfersContainer::updateTransfersVisibility(const Crypto::PublicKey &keyImage)
    {
