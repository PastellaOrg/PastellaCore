    ...
    class TransactionReaderListFormatter
    {
      public:
        explicit TransactionReaderListFormatter(
            const std::vector<std::unique_ptr<Pastella::ITransactionReader>> &transactionList):
      ...
      private:
        const std::vector<std::unique_ptr<Pastella::ITransactionReader>> &m_transactionList;
    };
...
namespace Pastella
{
    ...
    bool BlockchainSynchronizer::removeConsumer(IBlockchainConsumer *consumer)
    {
        ...
        if (result)
        {
            m_logger(INFO) << "Consumer removed, consumer " << consumer << ", count "
                                         << m_consumers.size();
        }
        else
        {
    ...
    void BlockchainSynchronizer::load(std::istream &in)
    {
        ...
        if (genesisBlockHash != m_genesisBlockHash)
        {
            ...
            m_logger(ERROR) << message << ", read " << genesisBlockHash << ", expected "
                                        << m_genesisBlockHash;
            throw std::runtime_error(message);
        }
        ...
        for (auto &kv : m_consumers)
        {
            ...
            if (result.hasNewBlocks)
            {
                ...
                if (addedCount > 0)
                {
                    ...
                    {
                        m_logger(ERROR) << "Failed to add " << (blockCount - addedCount) << " blocks of "
    ...
    void BlockchainSynchronizer::removeOutdatedTransactions()
    {
        ...
        if (!ec)
        {
            ...
            for (auto &consumer : m_consumers)
            {
                ...
                if (ec)
                {
                    m_logger(ERROR) << "Failed to process outdated pool transactions: " << ec << ", "
    ...
    std::error_code BlockchainSynchronizer::processPoolTxs(GetPoolResponse &response)
    {
        ...
        std::error_code error;
        {
            ...
            for (auto &consumer : m_consumers)
            {
                ...
                if (error)
                {
                    m_logger(ERROR) << "Failed to process pool transactions: " << error << ", "
