...
namespace Pastella
{
    ...
    NodeServer::NodeServer(
        ...
        Pastella::PastellaProtocolHandler &payload_handler,
        ...
        m_network_id(Pastella::PASTELLA_NETWORK),
        logger(log, "P2P.Node"),
        ...
        m_stop(false),
        // intervals
        // m_peer_handshake_idle_maker_interval(Pastella::P2P_DEFAULT_HANDSHAKE_INTERVAL),
        ...
        bool &handled)
    {
        ...
        if (cmd.isResponse && cmd.command == COMMAND_TIMED_SYNC::ID)
        {
            if (!handleTimedSyncResponse(cmd.buf, ctx))
            {
                ...
                ctx.m_state = PastellaConnectionContext::state_shutdown;
            }
            return 0;
        }
...
#undef INVOKE_HANDLER
    ...
    bool NodeServer::init_config()
    {
        try
        {
            ...
            m_config.m_net_config.handshake_interval = Pastella::P2P_DEFAULT_HANDSHAKE_INTERVAL;
            m_config.m_net_config.connections_count = Pastella::P2P_DEFAULT_CONNECTIONS_COUNT;
            m_config.m_net_config.packet_max_size = Pastella::P2P_DEFAULT_PACKET_MAX_SIZE; // 20 MB limit
            m_config.m_net_config.config_id = 0; // initial config
            m_config.m_net_config.connection_timeout = Pastella::P2P_DEFAULT_CONNECTION_TIMEOUT;
            m_config.m_net_config.ping_connection_timeout = Pastella::P2P_DEFAULT_PING_CONNECTION_TIMEOUT;
            m_config.m_net_config.send_peerlist_sz = Pastella::P2P_DEFAULT_PEERS_IN_HANDSHAKE;
    ...
    //-----------------------------------------------------------------------------------
    void NodeServer::for_each_connection(std::function<void(PastellaConnectionContext &, uint64_t)> f)
    {
        ...
        const std::list<boost::uuids::uuid> relayList)
    {
        m_dispatcher.remoteSpawn([this, command, data_buff, relayList] {
            forEachConnection([&](P2pConnectionContext &conn) {
                ...
                {
                    if (conn.peerId
                        && (conn.m_state == PastellaConnectionContext::state_normal
                            || conn.m_state == PastellaConnectionContext::state_synchronizing))
                    {
    ...
    {
        for (const auto &seed : Pastella::SEED_NODES)
        {
        ...
        // m_net_server.get_config_object().m_invoke_timeout = Pastella::P2P_DEFAULT_INVOKE_TIMEOUT;
    ...
    Pastella::PastellaProtocolHandler &NodeServer::get_payload_object()
    {
    ...
    //-----------------------------------------------------------------------------------
    bool NodeServer::handshake(Pastella::LevinProtocol &proto, P2pConnectionContext &context, bool just_take_peerlist)
    {
        ...
        if (rsp.node_data.version < Pastella::P2P_MINIMUM_VERSION)
        {
        ...
        else if ((rsp.node_data.version - Pastella::P2P_CURRENT_VERSION) >= Pastella::P2P_UPGRADE_WINDOW)
        {
            logger(Logging::WARNING) << context
                                     << "COMMAND_HANDSHAKE Warning, your software may be out of date. Please visit: "
                                     << Pastella::LATEST_VERSION_URL << " for the latest version.";
        }
    ...
    bool NodeServer::timedSync()
    {
        ...
        forEachConnection([&](P2pConnectionContext &conn) {
            if (conn.peerId
                && (conn.m_state == PastellaConnectionContext::state_normal
                    || conn.m_state == PastellaConnectionContext::state_idle))
            {
        ...
        try
        {
            ...
            try
            {
                System::Context<bool> handshakeContext(m_dispatcher, [&] {
                    Pastella::LevinProtocol proto(ctx.connection);
    ...
    bool NodeServer::connections_maker()
    {
        ...
        size_t expected_white_connections =
            (m_config.m_net_config.connections_count * Pastella::P2P_DEFAULT_WHITELIST_CONNECTIONS_PERCENT) / 100;
    ...
    {
        node_data.version = Pastella::P2P_CURRENT_VERSION;
        time_t local_time;
        time(&local_time);
...
//-----------------------------------------------------------------------------------
    ...
    bool NodeServer::check_trust(const proof_of_trust &tr)
    {
        ...
        Crypto::PublicKey pk;
        Common::podFromHex(Pastella::P2P_STAT_TRUSTED_PUB_KEY, pk);
        ...
        P2pConnectionContext &context)
    {
        ...
        if (arg.node_data.version < Pastella::P2P_MINIMUM_VERSION)
        {
        ...
        else if (arg.node_data.version > Pastella::P2P_CURRENT_VERSION)
        {
    ...
    void NodeServer::connectionHandler(const boost::uuids::uuid &connectionId, P2pConnectionContext &ctx)
    {
        ...
        System::Context<> context(m_dispatcher, [this, &connectionId, &ctx] {
            ...
            try
            {
                ...
                for (;;)
                {
                    if (ctx.m_state == PastellaConnectionContext::state_sync_required)
                    {
                        ctx.m_state = PastellaConnectionContext::state_synchronizing;
                        m_payload_handler.start_sync(ctx);
                    }
                    else if (ctx.m_state == PastellaConnectionContext::state_pool_sync_required)
                    {
                        ctx.m_state = PastellaConnectionContext::state_normal;
                    ...
                    if (ctx.m_state == PastellaConnectionContext::state_shutdown)
                    {
                        break;
                    }
                }
