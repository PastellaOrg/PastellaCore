...
namespace Pastella
{
    ...
    struct P2pConnectionContext : public PastellaConnectionContext
    {
      public:
        ...
        P2pConnectionContext(P2pConnectionContext &&ctx):
            PastellaConnectionContext(std::move(ctx)),
            context(ctx.context),
            peerId(ctx.peerId),
            connection(std::move(ctx.connection)),
            logger(ctx.logger.getLogger(), "P2P.Node"),
    ...
    class NodeServer : public IP2pEndpoint
    {
      public:
        ...
        Pastella::PastellaProtocolHandler &get_payload_object();
      ...
      private:
        ...
        bool
            handshake(Pastella::LevinProtocol &proto, P2pConnectionContext &context, bool just_take_peerlist = false);
        ...
        virtual bool invoke_notify_to_peer(
            ...
            const PastellaConnectionContext &context) override;

        virtual void
            for_each_connection(std::function<void(Pastella::PastellaConnectionContext &, uint64_t)> f) override;
        ...
        PastellaProtocolHandler &m_payload_handler;
