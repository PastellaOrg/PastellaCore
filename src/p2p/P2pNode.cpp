...
namespace Pastella
{
    ...
    bool P2pNode::fetchPeerList(ContextPtr connection)
    {
        try
        {
            ...
            if (response.node_data.version < Pastella::P2P_MINIMUM_VERSION)
            {
                logger(DEBUGGING) << *connection << "COMMAND_HANDSHAKE Failed, peer is wrong version: "
                                  << std::to_string(response.node_data.version);
                return false;
            }
            else if ((response.node_data.version - Pastella::P2P_CURRENT_VERSION) >= Pastella::P2P_UPGRADE_WINDOW)
            {
    ...
    basic_node_data P2pNode::getNodeData() const
    {
        ...
        nodeData.version = Pastella::P2P_CURRENT_VERSION;
    ...
    void P2pNode::handleNodeData(const basic_node_data &node, P2pContext &context)
    {
        ...
        if (node.version < Pastella::P2P_MINIMUM_VERSION)
        {
            std::ostringstream msg;
