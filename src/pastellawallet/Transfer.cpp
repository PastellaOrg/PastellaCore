...
void transfer(const std::shared_ptr<WalletBackend> walletBackend, const bool sendAll)
{
    ...
    const bool integratedAddressesAllowed(false), cancelAllowed(true);
    ...
    sendTransaction(walletBackend, address, amount, sendAll);
}
    ...
    const bool sendAll)
{
    ...
    else if (error == TOO_MANY_INPUTS_TO_FIT_IN_BLOCK)
    {
        std::cout << WarningMsg("Your transaction is too large to be accepted "
                                "by the network!\n")
                  << InformationMsg("The transaction is too large to fit in a block.\n"
                                    "Please try sending a smaller amount.\n\n");
    ...
    if (!confirmTransaction(walletBackend, address, actualAmount, nodeFee, preparedTransaction.fee))
    {
        cancel();
        return;
    }
    ...
    else
    {
        std::cout << SuccessMsg("\nTransaction has been sent!\n")
                  ...
                  << WhiteMsg("Link: ") << SuccessMsg("https://explorer.pastella.org/block/") << SuccessMsg(hash) << "\n" << std::endl;
    }
}
