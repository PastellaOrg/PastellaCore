...
#include "PastellaProtocolHandler.h"
...
namespace Pastella
{
    namespace
    {
        ...
        bool post_notify(
            ...
            const PastellaConnectionContext &context)
        {
    ...
    PastellaProtocolHandler::PastellaProtocolHandler(
    ...
    size_t PastellaProtocolHandler::getPeerCount() const
    {
        return m_peersCount;
    }

    void PastellaProtocolHandler::set_p2p_endpoint(IP2pEndpoint *p2p)
    {
        if (p2p)
        {
            m_p2p = p2p;
    ...
    void PastellaProtocolHandler::onConnectionClosed(PastellaConnectionContext &context)
    {
        ...
        if (updated)
        {
            ...
            m_observerManager.notify(&IPastellaProtocolObserver::lastKnownBlockHeightUpdated, m_observedHeight);
        }

        if (context.m_state != PastellaConnectionContext::state_befor_handshake)
        {
            m_peersCount--;
            m_observerManager.notify(&IPastellaProtocolObserver::peerCountUpdated, m_peersCount.load());
        }
    }

    void PastellaProtocolHandler::stop()
    {
        m_stop = true;
    }
    ...
    CoreStatistics PastellaProtocolHandler::getStatistics()
    {
        return m_core.getCoreStatistics();
    }

    void PastellaProtocolHandler::log_connections()
    {
        std::stringstream ss;
        ...
        m_p2p->for_each_connection([&](const PastellaConnectionContext &cntxt, uint64_t peer_id) {
    ...
    uint32_t PastellaProtocolHandler::get_current_blockchain_height()
    {
        return m_core.getTopBlockIndex() + 1;
    }

    bool PastellaProtocolHandler::process_payload_sync_data(
        const CORE_SYNC_DATA &hshd,
        PastellaConnectionContext &context,
        bool is_initial)
    {
        if (context.m_state == PastellaConnectionContext::state_befor_handshake && !is_initial)
        {
            return true;
        }
        ...
        else if (m_core.hasBlock(hshd.top_id))
        {
            if (is_initial)
            {
                ...
                context.m_state = PastellaConnectionContext::state_pool_sync_required;
            }
            else
            {
                context.m_state = PastellaConnectionContext::state_normal;
            }
        }
        else
        {
            ...
            ss << "Your " << COIN_NAME << " node is syncing with the network ";
            ...
            context.m_state = PastellaConnectionContext::state_sync_required;
        }
    ...
    bool PastellaProtocolHandler::get_payload_sync_data(CORE_SYNC_DATA &hshd)
    {
    ...
    int notifyAdaptor(const BinaryArray &reqBuf, PastellaConnectionContext &ctx, Handler handler)
    {
...
#define HANDLE_NOTIFY(CMD, Handler)                                                                           \
    case CMD::ID:                                                                                             \
    {                                                                                                         \
        ret = notifyAdaptor<CMD>(in, ctx, [this](int a1, CMD::request &a2, PastellaConnectionContext &a3) { \
            return Handler(a1, a2, a3);                                                                       \
        });                                                                                                   \
        break;                                                                                                \
    }

    int PastellaProtocolHandler::handleCommand(
        bool is_notify,
        int command,
        const BinaryArray &in,
        BinaryArray &out,
        PastellaConnectionContext &ctx,
        bool &handled)
    {
        int ret = 0;
        handled = true;
...
#undef HANDLE_NOTIFY
    ...
    int PastellaProtocolHandler::handle_notify_new_block(
        int command,
        ...
        PastellaConnectionContext &context)
    {
        ...
        if (context.m_state != PastellaConnectionContext::state_normal)
        {
            return 1;
        }
    ...
    int PastellaProtocolHandler::handle_notify_new_transactions(
        int command,
    ...
    int PastellaProtocolHandler::handle_request_get_objects(
        int command,
        ...
        PastellaConnectionContext &context)
    {
        ...
        //  connection"; context.m_state = PastellaConnectionContext::state_shutdown;
        //}
    ...
    int PastellaProtocolHandler::handle_response_get_objects(
        int command,
        ...
        PastellaConnectionContext &context)
    {
        ...
        for (size_t index = 0; index < rawBlocks.size(); ++index)
        {
            ...
            if (index == 1)
            {
                ...
                { // TODO
                    context.m_state = PastellaConnectionContext::state_idle;
        ...
        logger(DEBUGGING) << "Local blockchain updated, new index = " << m_core.getTopBlockIndex();
        if (!m_stop && context.m_state == PastellaConnectionContext::state_synchronizing)
        {
    ...
    int PastellaProtocolHandler::processObjects(
        PastellaConnectionContext &context,
        std::vector<RawBlock> &&rawBlocks,
    ...
    int PastellaProtocolHandler::doPushLiteBlock(
    ...
    int PastellaProtocolHandler::handle_request_chain(
        int command,
    ...
    bool PastellaProtocolHandler::request_missing_objects(
        PastellaConnectionContext &context,
        bool check_having_blocks)
    {
        ...
        else
        {
            ...
            logger(Logging::INFO, Logging::BRIGHT_GREEN)
                << context << "Successfully synchronized with the " << Pastella::COIN_NAME << " Network.";
            on_connection_synchronized();
        }
        return true;
    }

    bool PastellaProtocolHandler::on_connection_synchronized()
    {
        bool val_expected = false;
        if (m_synchronized.compare_exchange_strong(val_expected, true))
        {
            logger(Logging::INFO) << ENDL;
            logger(INFO, BRIGHT_MAGENTA) << "===[ " + std::string(Pastella::COIN_NAME)  + " has started! ]===============" << ENDL;
            logger(INFO, BRIGHT_MAGENTA) << "You are now ready to use the " + std::string(Pastella::COIN_NAME)  + " Node!" << ENDL;
            //logger(INFO) << "" << ENDL;
            logger(INFO, BRIGHT_MAGENTA) << "===========================================" << ENDL << ENDL;

            //logger(INFO) << asciiArt << ENDL;

            m_observerManager.notify(&IPastellaProtocolObserver::blockchainSynchronized, m_core.getTopBlockIndex());
        }
        return true;
    }

    int PastellaProtocolHandler::handle_response_chain_entry(
        int command,
    ...
    int PastellaProtocolHandler::handleRequestTxPool(
        int command,
    ...
    int PastellaProtocolHandler::handle_notify_new_lite_block(
        int command,
        NOTIFY_NEW_LITE_BLOCK::request &arg,
        PastellaConnectionContext &context)
    {
    ...
    int PastellaProtocolHandler::handle_notify_missing_txs(
        int command,
    ...
    void PastellaProtocolHandler::relayBlock(NOTIFY_NEW_BLOCK::request &arg)
    {
        ...
        m_p2p->for_each_connection([this, &liteBlockConnections, &normalBlockConnections](
                                       const PastellaConnectionContext &ctx, uint64_t peerId) {
    ...
    void PastellaProtocolHandler::relayTransactions(const std::vector<BinaryArray> &transactions)
    {
    ...
    void PastellaProtocolHandler::requestMissingPoolTransactions(const PastellaConnectionContext &context)
    {
    ...
    void
        PastellaProtocolHandler::updateObservedHeight(uint32_t peerHeight, const PastellaConnectionContext &context)
    {
        bool updated = false;
        {
            std::lock_guard<std::mutex> lock(m_observedHeightMutex);
    ...
    void PastellaProtocolHandler::recalculateMaxObservedHeight(const PastellaConnectionContext &context)
    {
        // should be locked outside
        uint32_t peerHeight = 0;
        m_p2p->for_each_connection([&peerHeight, &context](const PastellaConnectionContext &ctx, uint64_t peerId) {
    ...
    uint32_t PastellaProtocolHandler::getObservedHeight() const
    {
        std::lock_guard<std::mutex> lock(m_observedHeightMutex);
        return m_observedHeight;
    };

    uint32_t PastellaProtocolHandler::getBlockchainHeight() const
    {
        std::lock_guard<std::mutex> lock(m_blockchainHeightMutex);
        return m_blockchainHeight;
    };

    bool PastellaProtocolHandler::addObserver(IPastellaProtocolObserver *observer)
    {
        return m_observerManager.add(observer);
    }

    bool PastellaProtocolHandler::removeObserver(IPastellaProtocolObserver *observer)
    {
