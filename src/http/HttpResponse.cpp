...
namespace
{
    const char *getStatusString(Pastella::HttpResponse::HTTP_STATUS status)
    {
        switch (status)
        {
            case Pastella::HttpResponse::STATUS_200:
                return "200 OK";
            case Pastella::HttpResponse::STATUS_404:
                return "404 Not Found";
            case Pastella::HttpResponse::STATUS_500:
    ...
    const char *getErrorBody(Pastella::HttpResponse::HTTP_STATUS status)
    {
        switch (status)
        {
            case Pastella::HttpResponse::STATUS_404:
...
namespace Pastella
{
    ...
    {
        status = STATUS_200;
        headers["Server"] = "Pastella-based HTTP server";
