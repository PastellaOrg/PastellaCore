...
namespace
{
    void throwIfNotGood(std::istream &stream)
    {
        if (!stream.good())
        {
            if (stream.eof())
            {
                throw std::system_error(make_error_code(Pastella::error::HttpParserErrorCodes::END_OF_STREAM));
            }
            else
            {
                throw std::system_error(make_error_code(Pastella::error::HttpParserErrorCodes::STREAM_NOT_GOOD));
            }
        }
    }
...
namespace Pastella
{
    HttpResponse::HTTP_STATUS HttpParser::parseResponseStatusFromString(const std::string &status)
    {
        ...
        {
            return Pastella::HttpResponse::STATUS_200;
        }
        else if (status == "404 Not Found")
        {
            return Pastella::HttpResponse::STATUS_404;
        }
        else if (status == "500 Internal Server Error")
        {
            return Pastella::HttpResponse::STATUS_500;
        }
        else
        {
            throw std::system_error(
                make_error_code(Pastella::error::HttpParserErrorCodes::UNEXPECTED_SYMBOL),
    ...
    void HttpParser::readWord(std::istream &stream, std::string &word)
    {
        ...
        if (c == '\r')
        {
            ...
            if (c != '\n')
            {
                throw std::system_error(make_error_code(Pastella::error::HttpParserErrorCodes::UNEXPECTED_SYMBOL));
            }
        }
    }
    ...
    bool HttpParser::readHeader(std::istream &stream, std::string &name, std::string &value)
    {
        ...
        while (stream.good() && c != '\r')
        {
            if (c == ':')
            {
                ...
                if (name.empty())
                {
                    throw std::system_error(make_error_code(Pastella::error::HttpParserErrorCodes::EMPTY_HEADER));
                }
