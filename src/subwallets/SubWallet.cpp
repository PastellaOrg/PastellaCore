...
SubWallet::SubWallet(
    ...
    m_privateKey(Constants::NULL_SECRET_KEY),
...
/* TRANSPARENT SYSTEM: OBSOLETE FUNCTION - Returns empty (not needed in transparent system)
 *
 * Original: Derived ephemeral public/private key pair for transaction signing
 * Transparent system: NOT NEEDED - signing uses wallet's private key directly
 *
 * This function is a stub kept for API compatibility
 * Real signing happens in: Transfer.cpp:generateRingSignatures() using ownerPrivateKey
 *
 * Returns: Empty key pair (function is obsolete) */
std::tuple<Crypto::PublicKey, Crypto::SecretKey> SubWallet::getTxInputKeyImage(
    /* const Crypto::KeyDerivation derivation, - REMOVED */
    const size_t outputIndex) const
{
    /* Stub: Returns empty values - function is obsolete in transparent system */
    (void)outputIndex;

    return { Crypto::PublicKey(), Crypto::SecretKey() };
}

void SubWallet::storeTransactionInput(const WalletTypes::TransactionInput input)
{
    /* Find the input in the unconfirmed incoming amounts - inputs we
       sent ourselves, that are now returning as change. Remove from
       vector if found. */
    const auto unconfirmedIt = std::remove_if(
    ...
    if (unconfirmedIt != m_unconfirmedIncomingAmounts.end())
    {
        m_unconfirmedIncomingAmounts.erase(unconfirmedIt, m_unconfirmedIncomingAmounts.end());
    }

    /* TRANSPARENT SYSTEM: Check for duplicates by (parentTransactionHash, transactionIndex)
     * In transparent system, multiple outputs can go to the same address (same key).
     * We identify UTXOs by (transaction_hash, output_index), just like Bitcoin.
     *
     * Two outputs with the same key but different transactionIndex are DIFFERENT UTXOs! */
    auto it = std::find_if(m_unspentInputs.begin(), m_unspentInputs.end(), [&input](const auto x) {
        return x.parentTransactionHash == input.parentTransactionHash && x.transactionIndex == input.transactionIndex;
    });
    ...
    {
        std::stringstream ss;
        ss << "Adding new input to " << m_address << ": "
           << input.amount << " atomic units at block " << input.blockHeight
           << ", key: " << Common::podToHex(input.key).substr(0, 16) << "...";
        ...
        Logger::logger.log(
            "Total unspent inputs after adding: " + std::to_string(m_unspentInputs.size()),
            Logger::TRACE,
            {Logger::SYNC});
    }
    else
...
std::tuple<uint64_t, uint64_t> SubWallet::getBalance(const uint64_t currentHeight) const
{
    ...
    Logger::logger.log(
        "Calculating balance for " + m_address + " at height " + std::to_string(currentHeight),
        Logger::TRACE,
        {Logger::SYNC});

    Logger::logger.log(
        "Total unspent inputs: " + std::to_string(m_unspentInputs.size()),
        Logger::TRACE,
        {Logger::SYNC});
    ...
    for (const auto input : m_unspentInputs)
    {
        ...
        bool unlocked = Utilities::isInputUnlocked(input.unlockTime, currentHeight);
    ...
    Logger::logger.log(
        "Unconfirmed incoming amounts: " + std::to_string(m_unconfirmedIncomingAmounts.size()),
        Logger::TRACE,
        {Logger::SYNC});
    ...
    {
        Logger::logger.log(
            "Unconfirmed: " + std::to_string(unconfirmedInput.amount) + " atomic units",
            Logger::TRACE,
            {Logger::SYNC});
    ...
    std::stringstream finalBalance;
    finalBalance << "Final balance: unlocked=" << unlockedBalance
                  << " locked=" << lockedBalance
                  << " total=" << (unlockedBalance + lockedBalance);

    Logger::logger.log(finalBalance.str(), Logger::TRACE, {Logger::SYNC});
    Logger::logger.log("==================================================", Logger::TRACE, {Logger::SYNC});
...
Crypto::PublicKey SubWallet::publicKey() const
{
    return m_publicKey;
}

Crypto::SecretKey SubWallet::privateKey() const
{
    return m_privateKey;
}
...
void SubWallet::markInputAsSpent(const Crypto::Hash parentTransactionHash, const uint64_t transactionIndex, const uint64_t spendHeight)
{
    ...
    auto it = std::find_if(m_unspentInputs.begin(), m_unspentInputs.end(), [&parentTransactionHash, &transactionIndex](const auto x) {
        return x.parentTransactionHash == parentTransactionHash && x.transactionIndex == transactionIndex;
    });

    /* Check if this input is already in spentInputs */
    bool inSpent = std::find_if(m_spentInputs.begin(), m_spentInputs.end(), [&parentTransactionHash, &transactionIndex](const auto x) {
        return x.parentTransactionHash == parentTransactionHash && x.transactionIndex == transactionIndex;
    }) != m_spentInputs.end();
    ...
    it = std::find_if(
        m_lockedInputs.begin(), m_lockedInputs.end(), [&parentTransactionHash, &transactionIndex](const auto x) {
            return x.parentTransactionHash == parentTransactionHash && x.transactionIndex == transactionIndex;
        });
...
void SubWallet::markInputAsLocked(const Crypto::Hash parentTransactionHash, const uint64_t transactionIndex)
{
    std::stringstream ss;
    ss << "markInputAsLocked: parentTx=" << parentTransactionHash
     ...
     * This is necessary because multiple outputs can have the same key! */
    auto it = std::find_if(
        m_unspentInputs.begin(), m_unspentInputs.end(), [&parentTransactionHash, &transactionIndex](const auto x) {
            return x.parentTransactionHash == parentTransactionHash && x.transactionIndex == transactionIndex;
        });
    ...
    if (it == m_unspentInputs.end())
    {
        ...
        stream << "Could not find input with parentTx " << parentTransactionHash
               << " index " << transactionIndex << " to lock. Ignoring.";
    ...
    /* Check if this input is already in lockedInputs */
    bool inLocked = std::find_if(m_lockedInputs.begin(), m_lockedInputs.end(), [&parentTransactionHash, &transactionIndex](const auto x) {
        return x.parentTransactionHash == parentTransactionHash && x.transactionIndex == transactionIndex;
    }) != m_lockedInputs.end();
...
std::vector<WalletTypes::TxInputAndOwner> SubWallet::getSpendableInputs(const uint64_t height) const
{
    ...
    for (const auto input : m_unspentInputs)
    {
        bool unlocked = Utilities::isInputUnlocked(input.unlockTime, height);
        if (unlocked)
        {
            inputs.emplace_back(input, m_publicKey, m_privateKey);
        }
    }
    return inputs;
}
...
std::vector<Crypto::PublicKey> SubWallet::getKeyImages() const
{
    std::vector<Crypto::PublicKey> result;
    const auto getKeyImages = [&result](const auto &vec) {
        std::transform(
            vec.begin(), vec.end(), std::back_inserter(result), [](const auto &input) { return input.key; });
    };
...
void SubWallet::fromJSON(const JSONValue &j)
{
    ...
    m_publicKey.fromString(getStringFromJSON(j, "publicKey"));
    m_privateKey.fromString(getStringFromJSON(j, "privateKey"));
