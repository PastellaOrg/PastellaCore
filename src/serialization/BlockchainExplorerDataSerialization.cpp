...
namespace Pastella
{
    using Pastella::SerializationTag;

    namespace
    {
        struct BinaryVariantTagGetter : boost::static_visitor<uint8_t>
        {
            uint8_t operator()(const Pastella::BaseInputDetails)
            {
                return static_cast<uint8_t>(SerializationTag::Base);
            }

            uint8_t operator()(const Pastella::KeyInputDetails)
            {
        ...
        void getVariantValue(
            ...
            uint8_t tag,
            boost::variant<Pastella::BaseInputDetails, Pastella::KeyInputDetails> &in)
        {
            switch (static_cast<SerializationTag>(tag))
            {
                case SerializationTag::Base:
                {
                    Pastella::BaseInputDetails v;
                    serializer(v, "data");
                    in = v;
                    break;
                }
