...
/* STEALTH ADDRESS REMOVAL: Benchmark functions removed - KeyDerivation no longer exists
 * These functions tested stealth address key derivation performance
 * TODO: Consider adding new benchmarks for transparent address operations in Phase B */
/* STEALTH ADDRESS SYSTEM REMOVED: benchmarkUnderivePublicKey() removed
 * This function tested stealth address key derivation performance using:
 * - KeyDerivation (removed - no longer needed in transparent system)
 * - generate_key_derivation() (removed - stealth addresses removed)
 * - underive_public_key() (removed - stealth addresses removed)
 *
 * In the transparent Bitcoin-like system, outputs go directly to public keys
 * without key derivation, so this benchmark is no longer applicable.
 * TODO: Consider adding new benchmarks for transparent address operations.
 */

/* STEALTH ADDRESS SYSTEM REMOVED: benchmarkGenerateKeyDerivation() removed
 * This function tested stealth address key derivation performance using:
 * - KeyDerivation (removed - no longer needed in transparent system)
 * - generate_key_derivation() (removed - stealth addresses removed)
 *
...
void TestDeterministicSubwalletCreation (const std::string baseKey, const uint64_t subWalletIndex, const std::string expectedKey)
{
    Crypto::SecretKey f_baseKey;

    if (!Common::podFromHex(baseKey, f_baseKey))
    {

        exit(1);
    }

    Crypto::SecretKey f_expectedKey;

    if (!Common::podFromHex(expectedKey, f_expectedKey))
    {

        exit(1);
    }

    /* HD WALLET TEST REMOVED: deterministic subwallet functionality simplified in transparent system
    const auto [subwalletPrivateKey, subwalletPublicKey] = Crypto::generate_deterministic_subwallet_keys(f_baseKey, subWalletIndex);

    if (subwalletPrivateKey != f_expectedKey)
    {
    ...
    */
    ...
    std::cout << "HD wallet test skipped - deterministic subwallet functionality simplified." << std::endl;
}
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        if (o_benchmark)
        {
            ...
            /* STEALTH ADDRESS REMOVAL: Stealth address benchmark functions removed
             * benchmarkUnderivePublicKey() and benchmarkGenerateKeyDerivation() removed */
