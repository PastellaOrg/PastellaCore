...
namespace
{
    ...
    {
        std::cout << Pastella::storeToJson(obj) << ENDL;
        return true;
    }

    std::string printTransactionShortInfo(const Pastella::CachedTransaction &transaction)
    {
        std::stringstream ss;
    ...
    std::string printTransactionFullInfo(const Pastella::CachedTransaction &transaction)
    {
        std::stringstream ss;
        ss << printTransactionShortInfo(transaction);
        ss << "JSON: \n" << Pastella::storeToJson(transaction.getTransaction()) << std::endl;
...
{
    std::stringstream ss;
    ss << Pastella::COIN_NAME << " v" << PROJECT_VERSION_LONG << ENDL;
    ss << "Commands: " << ENDL;
...
bool DaemonCommandsHandler::print_tx(const std::vector<std::string> &args)
{
    ...
    std::vector<Pastella::BinaryArray> txs;
    std::vector<Crypto::Hash> missed_ids;
    m_core.getTransactions(tx_ids, txs, missed_ids);

    if (1 == txs.size())
    {
        Pastella::CachedTransaction tx(txs.front());
...
bool DaemonCommandsHandler::print_pool(const std::vector<std::string> &args)
{
    ...
    {
        Pastella::CachedTransaction ctx(tx);
