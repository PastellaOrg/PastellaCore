...
namespace DaemonConfig
{
    ...
    {
        cxxopts::Options options(argv[0], Pastella::getProjectCLIHeader());
        ...
        try
        {
            ...
            if (cli.count("rewind-to-height") > 0)
            {
                ...
                if (rewindHeight == 0)
                {
                    std::cout << Pastella::getProjectCLIHeader()
            ...
            {
                std::cout << Pastella::getProjectCLIHeader() << std::endl;
                exit(0);
            }
            else if (config.osVersion) // Do we want to display the OS version information?
            {
                std::cout << Pastella::getProjectCLIHeader() << "OS: " << Tools::get_os_version_string() << std::endl;
                exit(0);
            }
        }
