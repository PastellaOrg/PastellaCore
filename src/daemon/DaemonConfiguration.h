...
namespace DaemonConfig
{
    struct DaemonConfiguration
    {
        DaemonConfiguration()
        {
            ...
            logfile << Pastella::COIN_NAME << "d.log";

            dataDirectory = Tools::getDefaultDataDirectory();
            checkPoints = "default";
            logFile = logfile.str();
            logLevel = Logging::WARNING;
            dbMaxOpenFiles = Pastella::DATABASE_DEFAULT_MAX_OPEN_FILES;
            dbReadCacheSizeMB = Pastella::DATABASE_READ_BUFFER_MB_DEFAULT_SIZE;
            dbThreads = Pastella::DATABASE_DEFAULT_BACKGROUND_THREADS_COUNT;
            dbWriteBufferSizeMB = Pastella::DATABASE_WRITE_BUFFER_MB_DEFAULT_SIZE;
            rewindToHeight = 0;
            p2pInterface = "0.0.0.0";
            p2pPort = Pastella::P2P_DEFAULT_PORT;
            p2pExternalPort = 0;
            transactionValidationThreads = std::thread::hardware_concurrency();
            rpcInterface = "127.0.0.1";
            rpcPort = Pastella::RPC_DEFAULT_PORT;
            noConsole = false;
