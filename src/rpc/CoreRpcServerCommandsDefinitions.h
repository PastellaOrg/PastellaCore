...
#include <pastellacore/ICoreDefinitions.h>
...
#pragma pack(pop)
    ...
    /* UTXO Information for RPC output */
    struct UtxoInfo
    {
        std::string utxoKey;          /* UTXO identifier: txHash:outputIndex */
        std::string transactionHash;   /* Hash of transaction that created this UTXO */
        uint32_t outputIndex;          /* Index in transaction outputs */
        uint64_t amount;               /* Amount in atomic units */
        std::string publicKey;         /* Public key of output */
        ...
        void serialize(ISerializer &s)
        {
            ...
            KV_MEMBER(spent)
            KV_MEMBER(spentBlockIndex)
        }
    };

    /* RPC Command: Get all UTXOs in the network with pagination */
    struct COMMAND_RPC_GET_UTXOS
    {
        struct request
        {
            uint64_t page;      /* Page number (0-based) */
            uint64_t limit;     /* Number of UTXOs per page (max 1000) */
        ...
        struct response
        {
            std::vector<UtxoInfo> utxos;      /* Array of UTXOs */
            uint64_t totalUTXOs;               /* Total number of UTXOs in network */
            uint64_t totalPages;               /* Total number of pages */
            uint64_t currentPage;              /* Current page number */
            std::string status;                /* RPC status */

            void serialize(ISerializer &s)
            {
                KV_MEMBER(utxos)
                KV_MEMBER(totalUTXOs)
                KV_MEMBER(totalPages)
                KV_MEMBER(currentPage)
                KV_MEMBER(status)
            }
        };
    };
