...
namespace Utilities
{
        ...
        const uint64_t supported_height)
    {
        ...
        const float days = (next_fork - height) / Pastella::parameters::EXPECTED_NUMBER_OF_BLOCKS_PER_DAY;
    ...
    std::string get_fork_time(const uint64_t height, const std::vector<uint64_t> upgrade_heights)
    {
        ...
        const float days =
            (static_cast<float>(next_fork - height) / Pastella::parameters::EXPECTED_NUMBER_OF_BLOCKS_PER_DAY);
        ...
        for (auto upgrade : upgrade_heights)
        {
            if (upgrade > supported_height)
            {
                return "The network forked at height " + std::to_string(upgrade)
                       + ", please update your software: " + Pastella::LATEST_VERSION_URL;
            }
        }
