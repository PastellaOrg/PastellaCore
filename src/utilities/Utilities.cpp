...
namespace Utilities
{
    ...
    bool isInputUnlocked(const uint64_t unlockTime, const uint64_t currentHeight)
    {
        ...
        if (unlockTime >= Pastella::parameters::PASTELLA_MAX_BLOCK_NUMBER)
        {
            const uint64_t currentTimeAdjusted = static_cast<uint64_t>(std::time(nullptr))
                                                 + Pastella::parameters::PASTELLA_LOCKED_TX_ALLOWED_DELTA_SECONDS;

            return currentTimeAdjusted >= unlockTime;
        }

        const uint64_t currentHeightAdjusted =
            currentHeight + Pastella::parameters::PASTELLA_LOCKED_TX_ALLOWED_DELTA_BLOCKS;
    ...
    */
    ...
    {
        const uint64_t numerator = currentHeight * Pastella::parameters::MAX_BLOCK_SIZE_GROWTH_SPEED_NUMERATOR;
        const uint64_t denominator = Pastella::parameters::MAX_BLOCK_SIZE_GROWTH_SPEED_DENOMINATOR;

        const uint64_t growth = numerator / denominator;

        const uint64_t x = Pastella::parameters::MAX_BLOCK_SIZE_INITIAL + growth;
    ...
    uint64_t scanHeightToTimestamp(const uint64_t scanHeight)
    {
        ...
        uint64_t timestamp = Pastella::parameters::GENESIS_BLOCK_TIMESTAMP + secondsSinceLaunch;
    ...
    uint64_t timestampToScanHeight(const uint64_t timestamp)
    {
        ...
        if (timestamp <= Pastella::parameters::GENESIS_BLOCK_TIMESTAMP)
        {
            return 0;
        }

        /* Find the amount of seconds between launch and the timestamp */
        uint64_t launchTimestampDelta = timestamp - Pastella::parameters::GENESIS_BLOCK_TIMESTAMP;

        /* Get an estimation of the amount of blocks that have passed before the
           timestamp */
        return std::max<uint64_t>(0, (launchTimestampDelta / Pastella::parameters::DIFFICULTY_TARGET) - 10000);
    }
    ...
    bool parseDaemonAddressFromString(std::string &host, uint16_t &port, std::string address)
    {
        ...
        host = parts.at(0);
        port = Pastella::RPC_DEFAULT_PORT;
        ...
        const double feePerByte)
    {
        const uint64_t numChunks = static_cast<uint64_t>(std::ceil(
            transactionSize / static_cast<double>(Pastella::parameters::FEE_PER_BYTE_CHUNK_SIZE)
        ));
        uint64_t minFee = numChunks * feePerByte * Pastella::parameters::FEE_PER_BYTE_CHUNK_SIZE;
        ...
        const uint64_t height)
    {
        ...
        if (height < Pastella::parameters::MINIMUM_FEE_PER_BYTE_V1_HEIGHT && minFee < Pastella::parameters::MINIMUM_FEE) {
            minFee = Pastella::parameters::MINIMUM_FEE;
        }
        if (minFee > Pastella::parameters::ACCEPTABLE_FEE) minFee = Pastella::parameters::ACCEPTABLE_FEE;
        return minFee;
    }
        ...
        const size_t extraDataSize)
    {
        ...
        const size_t OUTPUT_KEY_SIZE = sizeof(decltype(Pastella::KeyOutput::key));
