...
namespace Pastella
{
        ...
        ITransfersSynchronizerObserver
    {
      public:
        ...
        virtual void initializeWithKey(
        ...
        virtual KeyPair getAddressKey(size_t index) const;

        virtual KeyPair getAddressKey(const std::string &address) const;

        virtual std::string createAddress();

        virtual std::string
            createAddress(const Crypto::SecretKey &secretKey, const uint64_t scanHeight, const bool newAddress);

        virtual std::string
            createAddress(const Crypto::PublicKey &publicKey, const uint64_t scanHeight, const bool newAddress);
      ...
      protected:
        ...
        Pastella::BlockDetails getBlock(const uint64_t blockHeight);
        ...
        {
            Pastella::AccountPublicAddress receiver;
...
#pragma pack(push, 1)
        struct ContainerStoragePrefix
        {
            ...
            EncryptedWalletRecord encryptedKeys;
        };
#pragma pack(pop)
        ...
        virtual void onBlockchainDetach(const Crypto::PublicKey &publicKey, uint32_t blockIndex) override;

        void blocksRollback(uint32_t blockIndex);

        virtual void
            onTransactionDeleteBegin(const Crypto::PublicKey &publicKey, Crypto::Hash transactionHash) override;

        void transactionDeleteBegin(Crypto::Hash transactionHash);

        virtual void
            onTransactionDeleteEnd(const Crypto::PublicKey &publicKey, Crypto::Hash transactionHash) override;
        ...
        void updateBalance(Pastella::ITransfersContainer *container);
        ...
        const WalletRecord &getWalletRecord(Pastella::ITransfersContainer *container) const;

        Pastella::AccountPublicAddress parseAddress(const std::string &address) const;
        ...
        void prepareTransaction(
            ...
            const Pastella::AccountPublicAddress &changeDestinationAddress,
        ...
        uint64_t countNeededMoney(const std::vector<Pastella::WalletTransfer> &destinations, uint64_t fee) const;

        Pastella::AccountPublicAddress parseAccountAddressString(const std::string &addressString) const;
        ...
        std::unique_ptr<Pastella::ITransaction> makeTransaction(
            const std::vector<ReceiverAmounts> &decomposedOutputs,
            std::vector<InputInfo> &keysInfo,
            const std::string &extra,
            uint64_t unlockTimestamp);

        void sendTransaction(const Pastella::Transaction &PastellaTransaction);
        ...
        void initBlockchain(const Crypto::PublicKey &publicKey);

        Pastella::AccountPublicAddress getChangeDestination(
        ...
        std::vector<Crypto::PublicKey> getPublicKeys() const;

        std::string getPrimaryAddress() const;

        std::vector<std::tuple<WalletTypes::TransactionInput, Crypto::Hash>>
            getInputs(const WalletRecord subWallet, const bool unspent) const;
        ...
        friend std::ostream &operator<<(std::ostream &os, Pastella::WalletGreen::WalletState state);
