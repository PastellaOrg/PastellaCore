...
namespace
{
    ...
    struct WalletTransactionDtoV2
    {
        ...
        WalletTransactionDtoV2(const Pastella::WalletTransaction &wallet)
        {
            state = wallet.state;
        ...
        Pastella::WalletTransactionState state;
    ...
    struct WalletTransferDtoV2
    {
        ...
        WalletTransferDtoV2(const Pastella::WalletTransfer &tr)
        {
            address = tr.address;
            amount = tr.amount;
    ...
    void serialize(UnlockTransactionJobDtoV2 &value, Pastella::ISerializer &serializer)
    {
    ...
    void serialize(WalletTransactionDtoV2 &value, Pastella::ISerializer &serializer)
    {
        typedef std::underlying_type<Pastella::WalletTransactionState>::type StateType;

        StateType state = static_cast<StateType>(value.state);
        serializer(state, "state");
        value.state = static_cast<Pastella::WalletTransactionState>(state);

        serializer(value.timestamp, "timestamp");
        Pastella::serializeBlockHeight(serializer, value.blockHeight, "blockHeight");
    ...
    void serialize(WalletTransferDtoV2 &value, Pastella::ISerializer &serializer)
    {
...
namespace Pastella
{
    ...
    {
        Pastella::BinaryInputStreamSerializer s(source);
    ...
    {
        Pastella::BinaryOutputStreamSerializer s(destination);
    ...
    void WalletSerializerV2::loadKeyListAndBalances(Pastella::ISerializer &serializer, bool saveCache)
    {
        ...
        for (size_t i = 0; i < walletCount; ++i)
        {
            ...
            cachedKeySet.insert(publicKey);

            auto it = index.find(publicKey);
            if (it == index.end())
            {
                m_deletedKeys.emplace(std::move(publicKey));
            }
            else if (saveCache)
            {
                m_actualBalance += actualBalance;
        ...
        {
            if (cachedKeySet.count(wallet.publicKey) == 0)
            {
                m_addedKeys.insert(wallet.publicKey);
            }
        }
    }

    void WalletSerializerV2::saveKeyListAndBalances(Pastella::ISerializer &serializer, bool saveCache)
    {
        ...
        {
            serializer(wallet.publicKey, "publicKey");
    ...
    void WalletSerializerV2::loadTransactions(Pastella::ISerializer &serializer)
    {
        uint64_t count = 0;
    ...
    void WalletSerializerV2::saveTransactions(Pastella::ISerializer &serializer)
    {
    ...
    void WalletSerializerV2::loadTransfers(Pastella::ISerializer &serializer)
    {
        uint64_t count = 0;
    ...
    void WalletSerializerV2::saveTransfers(Pastella::ISerializer &serializer)
    {
    ...
    void WalletSerializerV2::loadTransfersSynchronizer(Pastella::ISerializer &serializer)
    {
    ...
    void WalletSerializerV2::saveTransfersSynchronizer(Pastella::ISerializer &serializer)
    {
    ...
    void WalletSerializerV2::loadUnlockTransactionsJobs(Pastella::ISerializer &serializer)
    {
    ...
    void WalletSerializerV2::saveUnlockTransactionsJobs(Pastella::ISerializer &serializer)
    {
        auto &index = m_unlockTransactions.get<TransactionHashIndex>();
        ...
        for (const auto &j : index)
        {
            ...
            dto.walletSpendPublicKey = keyIt->publicKey;
