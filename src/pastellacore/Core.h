    ...
    class Core : public ICore, public ICoreInformation
    {
      public:
        ...
        virtual std::tuple<bool, std::vector<UtxoOutput>, uint64_t>
            getUTXOs(uint64_t page, uint64_t limit) const override;
        ...
        static WalletTypes::RawCoinbaseTransaction getRawCoinbaseTransaction(const Pastella::Transaction &t);
        ...
        virtual bool reactivateStakesForRewind(uint64_t targetHeight);

        virtual std::vector<MaturedStakeReward> getMaturedStakeRewards(uint32_t blockHeight);

        virtual std::vector<Pastella::RichListEntry> getRichList(size_t count) const override;

        virtual Pastella::WalletDetails getWalletDetails(
            const std::string &address,
            size_t limit = 100,
            size_t page = 0) const override;

        /* UTXO SYSTEM: UTXO query method overrides */
      ...
      private:
        ...
        std::unique_ptr<GovernanceManager> governanceManager;
        ...
        void scanAndRestoreStakingTransactions(uint64_t currentHeight);
