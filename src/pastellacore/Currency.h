...
namespace Pastella
{
    /* Structure to hold matured stake reward information for Phase 2 */
    /* TRANSPARENT SYSTEM: stakerAddress is the wallet that created the stake */
    struct MaturedStakeReward
    {
        std::string stakerAddress;   // Wallet address that created the stake
        uint64_t rewardAmount;        // Calculated reward amount in atomic units
        std::string stakeTxHash;      // Original staking transaction hash (for reference)
    };
    ...
    class Currency
    {
      public:
        ...
        {
            if (height >= Pastella::parameters::DUST_THRESHOLD_V2_HEIGHT)
            {
                return Pastella::parameters::DEFAULT_DUST_THRESHOLD_V2;
            }
        ...
        uint64_t calculateBlockRewardByHeight(uint32_t height) const;

        uint64_t calculateTotalGeneratedCoins(uint32_t height, uint64_t totalPaidStakingRewards = 0) const;
        ...
        bool checkProofOfWorkV1(const CachedBlock &block, uint64_t currentDifficulty, uint32_t blockHeight) const;
        ...
        bool checkProofOfWork(const CachedBlock &block, uint64_t currentDifficulty, uint32_t blockHeight) const;
