...
void RocksDBWrapper::init(const DataBaseConfig &config)
{
    ...
    {
        throw std::system_error(make_error_code(Pastella::error::DataBaseErrorCodes::ALREADY_INITIALIZED));
    }
    ...
    else if (!status.ok() && status.IsInvalidArgument())
    {
        ...
        if (!status.ok())
        {
            ...
            throw std::system_error(make_error_code(Pastella::error::DataBaseErrorCodes::INTERNAL_ERROR));
        }
    }
    else if (status.IsIOError())
    {
        logger(ERROR) << "DB Error. DB can't be opened in " << dataDir << ". Error: " << status.ToString();
        throw std::system_error(make_error_code(Pastella::error::DataBaseErrorCodes::IO_ERROR));
    }
    else
    {
        logger(ERROR) << "DB Error. DB can't be opened in " << dataDir << ". Error: " << status.ToString();
...
void RocksDBWrapper::shutdown()
{
    ...
    {
        throw std::system_error(make_error_code(Pastella::error::DataBaseErrorCodes::NOT_INITIALIZED));
    }
