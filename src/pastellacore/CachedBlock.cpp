...
const Crypto::Hash &CachedBlock::getBlockLongHash() const
{
    ...
    try
    {
        if (block.majorVersion >= Pastella::BLOCK_MAJOR_VERSION_1) {
            // For RandomX blocks, use height-aware hashing
            uint32_t height = getBlockIndex();
            Crypto::randomx_slow_hash_with_height(rawHashingBlock.data(), rawHashingBlock.size(), height, blockLongHash.get());
        } else {
            // For non-RandomX blocks, use the configured algorithm
            const auto hashingAlgorithm = Pastella::HASHING_ALGORITHMS_BY_BLOCK_VERSION.at(block.majorVersion);
...
uint32_t CachedBlock::getBlockIndex() const
{
    ...
    {
        if (block.baseTransaction.inputs.empty())
        {
            blockIndex = 0;
        }
        else
