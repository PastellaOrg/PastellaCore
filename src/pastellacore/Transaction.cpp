...
namespace Pastella
{
    ...
    class TransactionImpl : public ITransaction
    {
      public:
        ...
        TransactionImpl(const Pastella::Transaction &tx);
    ...
    std::unique_ptr<ITransaction> createTransaction(const Pastella::Transaction &tx)
    {
    ...
    {
        Pastella::KeyPair txKeys(Pastella::generateKeyPair());
    ...
    TransactionImpl::TransactionImpl(const Pastella::Transaction &tx): transaction(tx)
    {
        extra.parse(transaction.extra);
    }
    ...
    void TransactionImpl::signInputKey(size_t index, const TransactionTypes::InputKeyInfo &info, const KeyPair &ephKeys)
    {
        ...
        const auto [success, signatures] = crypto_ops::generateRingSignatures(
            prefixHash, Crypto::PublicKey() /* Not needed - Ed25519 uses direct key */, publicKeys, ephKeys.secretKey, info.realOutput.transactionIndex);
    ...
    {
        return ::Pastella::findOutputsToAccount(transaction, addr, out, amount);
    }
