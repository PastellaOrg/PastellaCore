...
namespace Pastella
{
    namespace DB
    {
        ...
        /* UTXO SYSTEM: Database prefixes for UTXO tracking in transparent system
         *
         * UTXO_KEY_TO_UTXO_PREFIX: Maps (transactionHash, outputIndex) → UtxoOutput
         *   - Primary UTXO lookup key
         *   - Used to check if UTXO exists and its spent status
         *   - Key format: "h" + transactionHash(32 bytes) + outputIndex(4 bytes)
         *
         * UTXO_SPENT_PREFIX: Maps spent UTXO keys to block index where spent
         *   - Tracks spent UTXOs for reorg handling
         *   - Key format: "i" + transactionHash(32 bytes) + outputIndex(4 bytes)
         *   - Value: blockIndex where UTXO was spent
         */

        const std::string UTXO_KEY_TO_UTXO_PREFIX = "h";
        ...
        {
            Pastella::KVBinaryOutputStreamSerializer serializer;
            std::stringstream ss;
        ...
        template<class Value> void deserialize(const std::string &serialized, Value &value, const std::string &name)
        {
            ...
            Pastella::KVBinaryInputStreamSerializer serializer(stream);
