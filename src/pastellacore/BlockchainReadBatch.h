...
namespace std
{
    template<> struct hash<std::pair<Pastella::IBlockchainCache::Amount, uint32_t>>
    {
        using argment_type = std::pair<Pastella::IBlockchainCache::Amount, uint32_t>;
...
namespace Pastella
{
    ...
    struct BlockchainReadState
    {
        ...
        std::unordered_map<uint32_t, std::vector<Crypto::PublicKey>> spentKeyImagesByBlock;

        /* STEALTH ADDRESS REMOVAL: Changed KeyImage to PublicKey */
        std::unordered_map<Crypto::PublicKey, uint32_t> blockIndexesBySpentKeyImages;
        ...
        /* UTXO SYSTEM: UTXO database read state
         *
         * utxos: Map of (transactionHash, outputIndex) -> UtxoOutput
         *        Contains complete UTXO data loaded from database
         *
         * spentUtxos: Map of (transactionHash, outputIndex) -> spentBlockIndex
         *             Tracks which block spent each UTXO (for reorg handling) */
        std::unordered_map<std::pair<Crypto::Hash, uint32_t>, UtxoOutput> utxos;

        std::unordered_map<std::pair<Crypto::Hash, uint32_t>, uint32_t> spentUtxos;
    ...
    class BlockchainReadResult
    {
      public:
        ...
        const std::unordered_map<uint32_t, std::vector<Crypto::PublicKey>> &getSpentKeyImagesByBlock() const;

        const std::unordered_map<Crypto::PublicKey, uint32_t> &getBlockIndexesBySpentKeyImages() const;
        ...
        /* UTXO SYSTEM: UTXO getter methods */
        const std::unordered_map<std::pair<Crypto::Hash, uint32_t>, UtxoOutput> &getUtxos() const;

        const std::unordered_map<std::pair<Crypto::Hash, uint32_t>, uint32_t> &getSpentUtxos() const;
    ...
    class BlockchainReadBatch : public IReadBatch
    {
      public:
        ...
        BlockchainReadBatch &requestBlockIndexBySpentKeyImage(const Crypto::PublicKey &keyImage);
        ...
        /* UTXO SYSTEM: Database read request methods */
        BlockchainReadBatch &requestUtxo(
            const Crypto::Hash &transactionHash,
            uint32_t outputIndex);

        BlockchainReadBatch &requestUtxosForTransaction(
            const Crypto::Hash &transactionHash,
            uint32_t outputCount);

        /* Request all UTXOs from database */
        BlockchainReadBatch &requestAllUtxos();

        BlockchainReadBatch &requestSpentUtxo(
            const Crypto::Hash &transactionHash,
            uint32_t outputIndex);
