    ...
    m_rpcPassword(rpcPassword)
{
    ...
    m_server
        ...
        .Get(
            ...
            router(&ApiDispatcher::getKeys, WalletMustBeOpen, viewWalletsBanned))
    ...
    ApiDispatcher::keyImportWallet(const httplib::Request &req, httplib::Response &res, const nlohmann::json &body)
{
    ...
    const auto [daemonHost, daemonPort, daemonSSL, filename, password] = getDefaultWalletParams(body);
    const auto privateKey = getJsonValue<Crypto::SecretKey>(body, "privateKey");
...
{
    const auto [error, address, privateKey, subWalletIndex] = m_walletBackend->addSubWallet();
    const auto publicKey = Utilities::addressToPublicKey(address);

    nlohmann::json j {{"address", address}, {"privateKey", privateKey}, {"publicKey", publicKey}, {"walletIndex", subWalletIndex}};
...
std::tuple<Error, uint16_t> ApiDispatcher::importAddress(const httplib::Request &req, httplib::Response &res, const nlohmann::json &body)
{
    ...
    const auto [error, address] = m_walletBackend->importSubWallet(privateKey, scanHeight);
    ...
    ApiDispatcher::validateAddress(const httplib::Request &req, httplib::Response &res, const nlohmann::json &body)
{
    ...
    const bool isIntegrated = false; /* Always false in transparent system */
    const auto publicKey = Utilities::addressToPublicKey(actualAddress);

    nlohmann::json j {
        {"isIntegrated", address.length() == WalletConfig::standardAddressLength},
...
std::tuple<Error, uint16_t> ApiDispatcher::setNodeInfo(const httplib::Request &req, httplib::Response &res, const nlohmann::json &body)
{
    ...
    uint16_t daemonPort = Pastella::RPC_DEFAULT_PORT;
...
std::tuple<Error, uint16_t>
    ApiDispatcher::getKeys(const httplib::Request &req, httplib::Response &res, const nlohmann::json &body) const
{
    ...
    const auto [error, publicKey, privateKey, walletIndex] = m_walletBackend->getKeys(address);
    ...
    nlohmann::json j {{"publicKey", publicKey}, {"privateKey", privateKey}, {"walletIndex", walletIndex}};
    ...
    const nlohmann::json &body) const
{
    ...
    for (const auto tx : m_walletBackend->getTransactions())
    {
        if (tx.hash == hash)
        {
            ...
            {
                Crypto::PublicKey publicKey = tx.at("publicKey").get<Crypto::PublicKey>();
...
{
    /* Always return true - no view wallets in transparent system */
    return true;
}
bool ApiDispatcher::assertIsViewWallet() const
{
    /* Always return false - no view wallets in transparent system */
    return false;
}
