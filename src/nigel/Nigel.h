...
class Nigel
{
  public:
    ...
    std::tuple<bool, bool, std::string> sendTransaction(const Pastella::Transaction tx) const;
    ...
    /* Get user staking positions from daemon */
    std::tuple<bool, std::vector<WalletTypes::StakeInfo>> getUserStakesByHashes(
    ...
    std::tuple<bool, std::string> createProposal(
    ...
    std::tuple<bool, uint64_t> getNextProposalId() const;

    /* Cast a vote on a proposal */
    std::tuple<bool, std::string> castVote(
        const uint64_t proposalId,
        const uint8_t vote) const;

    /* Get all governance proposals */
    std::tuple<bool, std::vector<WalletTypes::GovernanceProposal>> getGovernanceProposals(
        const bool activeOnly = false) const;

    /* Get specific proposal details */
    std::tuple<bool, WalletTypes::GovernanceProposal, std::vector<WalletTypes::GovernanceVote>> getGovernanceProposal(
        const uint64_t proposalId) const;

    /* Get voting power for an address */
    std::tuple<bool, uint64_t, std::vector<WalletTypes::StakeInfo>> getVotingPower(
        const std::string address) const;

    /* Get raw transaction data from daemon */
    std::tuple<bool, std::vector<uint8_t>> getTransaction(
  ...
  private:
    ...
    std::atomic<uint64_t> m_blockCount = Pastella::BLOCKS_SYNCHRONIZING_DEFAULT_COUNT;
